...
class ADAINDecoder(nn.Module):
    ...
    def __init__(self, pose_nc, ngf, img_f, encoder_layers, decoder_layers, skip_connect=True,
        ...
        for i in range(encoder_layers - decoder_layers, encoder_layers)[::-1]:
            ...
            model = ADAINDecoderBlock(in_channels, out_channels, out_channels, pose_nc, use_transpose, nonlinearity,
                                      use_spect)
...
class ADAINDecoderBlock(nn.Module):
    def __init__(self, input_nc, output_nc, hidden_nc, feature_nc, use_transpose=True, nonlinearity=nn.LeakyReLU(),
                 use_spect=False):
...
class FineEncoder(nn.Module):
    ...
    def __init__(self, image_nc, ngf, img_f, layers, norm_layer=nn.BatchNorm2d, nonlinearity=nn.LeakyReLU(),
                 use_spect=False):
...
class FineDecoder(nn.Module):
    ...
    def __init__(self, image_nc, feature_nc, ngf, img_f, layers, num_block, norm_layer=nn.BatchNorm2d,
                 nonlinearity=nn.LeakyReLU(), use_spect=False):
...
class FineDecoderV2(nn.Module):
    """docstring for FineDecoder"""

    def __init__(self, image_nc, feature_nc, ngf, img_f, layers, num_block, norm_layer=nn.BatchNorm2d,
                 nonlinearity=nn.LeakyReLU(), use_spect=False):
        super(FineDecoderV2, self).__init__()
        self.layers = layers
        for i in range(layers)[::-1]:
            in_channels = min(ngf * (2 ** (i + 1)), img_f)
            out_channels = min(ngf * (2 ** i), img_f)
        ...
        self.final1 = FinalBlock2d(out_channels, image_nc, use_spect, 'tanh')
        self.final2 = FinalBlock2d(out_channels, image_nc, use_spect, 'tanh')
    ...
    def forward(self, x, z):
        ...
        for i in range(self.layers)[::-1]:
            ...
            out = up_model(out)
            out = jump_model(x.pop()) + out
        out_image1 = self.final1(out)
        out_image2 = self.final2(out)
...
class FineADAINResBlocks(nn.Module):
    def __init__(self, num_block, input_nc, feature_nc, norm_layer=nn.BatchNorm2d, nonlinearity=nn.LeakyReLU(),
                 use_spect=False):
...
class FineADAINResBlock2d(nn.Module):
    ...
    def forward(self, x, z):
        x = self.actvn(self.norm1(self.conv1(x), z))
