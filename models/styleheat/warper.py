...
def test_sync_net():
    from loss.sync_loss import SyncNet
    syncnet = SyncNet()
    path = '/apdcephfs/share_1290939/feiiyin/TH/ft_local/lipsync_expert.pth'
    ckpt = torch.load(path, map_location='cpu')['state_dict']
    syncnet.load_state_dict(ckpt)
    ...
    mel = torch.randn(2, 1, 80, 16).to(device)
    g = torch.randn(2, 3, 5, 96, 96).to(device)  # need to crop face and resize to 96 * 96
    sync_loss = get_sync_loss(mel, g)
    print(sync_loss.shape, sync_loss)
