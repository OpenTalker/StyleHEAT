...
def align_image(images, lms):
    """
    Input: images PIL list
    lms batch tensor
    """
    ...
    for i, image in enumerate(images):
        lm = lms[i]
        align = align_face_single(image, lm, output_size=256)
        output_images.append(align)

    output_images = [transform(i) for i in output_images]
    output_images = torch.stack(output_images).float()
    return output_images


def align_image_pil(images, lms):
    """
    Input: images PIL list
    lms batch tensor
    """
